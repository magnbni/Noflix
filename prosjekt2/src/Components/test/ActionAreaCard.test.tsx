...
describe('ActionAreaCard', () => {
  ...
  it('renders correctly with a movie', () => {
    const { getByText, getByTestId } = render(<ActionAreaCard movie={mockMovie} />);
    expect(getByText('Example Movie Title')).toBeInTheDocument();

    // Check for the date
    const formattedDate = new Date(mockMovie.releaseDate).toLocaleDateString('en-GB');
    expect(getByText(formattedDate)).toBeInTheDocument();

    // Check for the image
    const image = getByTestId('movie-poster'); // Make sure to add 'data-testid="movie-poster"' to your image component
    expect(image).toHaveStyle(`backgroundImage: url(https://image.tmdb.org/t/p/original//${mockMovie.posterPath})`);
  });
